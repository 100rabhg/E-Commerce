...
RSpec.describe "Products", type: :request do
  describe "GET show" do
    context "visit products show page" do
      ...
      @product = FactoryBot.create(:product, store: store, category: category)
  ...
  describe "post create" do
    context "when user is authenticate" do
      ...
      it "return a successful if have store" do
        store = FactoryBot.create(:store, user: @user)
        category = FactoryBot.create(:category)
        post products_path, params: { product: {title: "Product title", price:10 ,quantity: 12, store: store , category_id: category.id}}
