  ...
  describe 'put update' do
    context 'if user is login' do
      ...
      it 'return a successful responce' do
        ...
        store = FactoryBot.create(:store, user: @user)
        product = FactoryBot.create(:product, category:, store:)
        cartItem = FactoryBot.create(:cartItem, product:, user: @user)
  ...
  describe 'post order' do
    context 'if user is login' do
      ...
      it 'return a successful responce' do
        ...
        store = FactoryBot.create(:store, user: @user)
        product = FactoryBot.create(:product, category:, store:)
        FactoryBot.create(:cartItem, product:, user: @user)
        expect do
          post order_cart_index_path,
               params: { order: { mobile_number: '1234567890', address: 'asdfv', pincode: '123456', state: 'sxcfdx',
                                  payment: :COD } }
